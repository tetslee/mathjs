      ...
      switch (node.name) {
        ...
        case 'log':
          if (!arg2 && node.args.length == 1) {
            ...
            funcDerivative = new OperatorNode('*', 'multiply', [
              arg1.clone(),
              new FunctionNode(new SymbolNode('log'), [arg2 || node.args[1]])
            ]);
          } else {
            // d/dx(log(f(x), g(x))) = d/dx(log(f(x)) / log(g(x)))
            return _derivative(new OperatorNode('/', 'divide', [
                new FunctionNode(new SymbolNode('log'), [arg1]),
                new FunctionNode(new SymbolNode('log'), [node.args[1]])
              ]), constNodes);
          }
          ...
          if (constNodes[arg1] !== undefined) {
            ...
            return new OperatorNode('*', 'multiply', [
              ...
              new OperatorNode('*', 'multiply', [
                new FunctionNode(new SymbolNode('log'), [arg1.clone()]),
